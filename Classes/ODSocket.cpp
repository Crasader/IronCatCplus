...
#else 
    bind(m_sock, (struct sockaddr*)&svraddr, sizeof(svraddr));
#endif*/
	return true;
}
//for server
...
int ODSocket::Send(ByteBuffer* data, int flags)
{
	int len = data->size() +4;
	data->writeInt(0, data->size());
	int count = this->Send(data->flush(), len, flags);

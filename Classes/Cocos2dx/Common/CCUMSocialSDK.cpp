...
CCUMSocialSDK::CCUMSocialSDK(const char* appKey) :
		_wrapperType("Cocos2d-x"), _wrapperVersion(
				"2.0") {
 ...
 */
void CCUMSocialSDK::authorize(int platform, std::function<void(int,int,const map<string, string>&)> callback) {
#if CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID
	// 授权某平台
...
void CCUMSocialSDK::deleteAuthorization(int platform,
		std::function<void(int,int,const map<string, string>&)> callback) {
#if CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID
...
void CCUMSocialSDK::openShare(const char* text, const char* imgName,
		std::function<void(int,int,const string&)> callback) {
#if CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID
	// 设置分享内容
...
void CCUMSocialSDK::directShare(int platform, const char* text,
		const char* imgName, std::function<void(int,int,const string&)> callback) {
#if CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID
